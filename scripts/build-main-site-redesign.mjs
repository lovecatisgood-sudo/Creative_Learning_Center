...
const CODING_CSS_VERSION = "20260819-coding-layout-v1";
...
function layout({ page, titleEn, titleTh, description, descriptionTh, body, active = page, extraHead = "", indexable = true, follow = true, ogImage = "https://creative.siamesecat.cafe/landing/og-siamese-cat-creative-club.jpg", ogImageWidth = 1200, ogImageHeight = 630, ogImageAlt = "Siamese Cat Creative Club" }) {
  ...
  const pageAssetVersion = page === "coding-with-ai" ? CODING_CSS_VERSION : ASSET_VERSION;
...
const contact = layout({
  ...
  body: `<section class="hero" style="min-height:560px"><div class="container hero-grid"><div class="hero-copy reveal visible"><span class="eyebrow">${text("Contact Us", "ติดต่อเรา")}</span><h1>${text("Tell us how we can help", "บอกเราได้เลยว่าต้องการให้ช่วยเรื่องใด")}</h1><p class="lead">${text("Send your question or tell us which service you are interested in. Our team will reply using the phone number or email you provide.", "ส่งคำถามหรือแจ้งบริการที่สนใจ ทีมงานจะติดต่อกลับทางเบอร์โทรหรืออีเมลที่คุณให้ไว้")}</p><div class="hero-actions"><a class="btn btn-line" href="https://wa.me/66953455767" target="_blank" rel="noopener">${text("WhatsApp Us", "ติดต่อทาง WhatsApp")}</a><a class="btn btn-secondary" href="mailto:Cafe@siamesecat.cafe">Cafe@siamesecat.cafe</a></div></div><div class="hero-art reveal visible"><div class="gallery-item environment-card" style="width:min(100%,520px);min-height:520px">${imageTag({ file: "environment-shop-front.webp", className: "environment-photo", alt: text("Siamese Cat Creative Club entrance", "ทางเข้า Siamese Cat Creative Club"), eager: true })}<div class="caption"><strong>${text("Near Mega Bangna", "ใกล้เมกาบางนา")}</strong></div></div></div></div></section>
<section class="section fawn"><div class="container booking-layout"><aside class="contact-panel"><div class="card contact-method-card"><span class="eyebrow">${text("Direct contact", "ช่องทางติดต่อโดยตรง")}</span><h2>${text("Reach our team", "ติดต่อทีมงาน")}</h2><div class="contact-method-list"><a href="https://wa.me/66953455767" target="_blank" rel="noopener"><strong>WhatsApp</strong><span>+66 95 345 5767</span></a><a href="mailto:Cafe@siamesecat.cafe"><strong>${text("Email", "อีเมล")}</strong><span>Cafe@siamesecat.cafe</span></a><a href="tel:+66953455767"><strong>${text("Telephone", "โทรศัพท์")}</strong><span>+66 95 345 5767</span></a><a href="${CAFE_URL}"><strong>Siamese Cat Cafe</strong><span>siamesecat.cafe</span></a></div></div></aside>
