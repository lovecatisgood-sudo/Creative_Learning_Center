...
function toolNavigation(language, path) {
  ...
  return `<header class="tool-navigation"><div class="tool-navigation__inner"><a class="tool-navigation__brand" href="${home}"><img src="/tools/kids-routine-chart/assets/creative-club-logo.webp" width="48" height="48" alt="Siamese Cat Creative Club"><span><strong>Siamese Cat Creative Club</strong><span>${isEnglish ? "Flexible • Creative • Caring" : "ยืดหยุ่น • สร้างสรรค์ • ใส่ใจ"}</span></span></a><nav class="tool-navigation__links" aria-label="${isEnglish ? "Main navigation" : "เมนูหลัก"}"><a href="${inside}">${isEnglish ? "Inside the Club" : "ภายในคลับ"}</a><a href="${creative}">${isEnglish ? "Creative Club" : "ครีเอทีฟคลับ"}</a><a href="${playgroup}">${isEnglish ? "Playgroup" : "เพลย์กรุ๊ป"}</a><a href="${blog}">${isEnglish ? "Blog" : "บล็อก"}</a><a href="${tools}" aria-current="page">${isEnglish ? "Free Tools" : "เครื่องมือฟรี"}</a><a href="${contact}">${isEnglish ? "Contact" : "ติดต่อเรา"}</a><a class="tool-navigation__language" href="${alternate}" hreflang="${isEnglish ? "th" : "en"}" lang="${isEnglish ? "th" : "en"}">${isEnglish ? "ไทย" : "English"}</a></nav></div></header>`;
}
